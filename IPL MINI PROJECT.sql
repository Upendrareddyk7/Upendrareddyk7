...
select a.bidder_id,count(a.bid_status),b.no_of_bids,a.bid_status,
(count(a.bid_status)/b.no_of_bids)*100 as 'percentage_win'
from ipl_bidding_details a
inner join ipl_bidder_points b
on a.bidder_id=b.bidder_id
and a.bid_status='won'
group by a.bidder_id,b.no_of_bids
...
SELECT TEAM_ID, TEAM_NAME, TEAM_ID1, MATCH_WINNER,B.WIN_DETAILS
FROM IPL_TEAM A
INNER JOIN IPL_MATCH B
ON SUBSTR(A.REMARKS,1,2) = SUBSTR(B.WIN_DETAILS,6,2);
...
SELECT A.TEAM_NAME,B.PLAYER_ROLE,
COUNT(B.PLAYER_ROLE) AS TOTAL_ALLROUNDERS
FROM ipl_team A,ipl_team_players B 
WHERE A.TEAM_ID=B.TEAM_ID 
AND B.PLAYER_ROLE='ALL-ROUNDER' 
GROUP BY 1 
HAVING TOTAL_ALLROUNDERS > 4
